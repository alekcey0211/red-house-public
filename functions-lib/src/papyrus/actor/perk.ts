...
export const hasPerk = (mp: Mp, self: PapyrusObject, args: PapyrusValue[]): boolean => {
...
export const addPerk = (mp: Mp, self: PapyrusObject, args: PapyrusValue[]): void => {
	const perk = getObject(args, 0);
	const selfId = mp.getIdFromDesc(self.desc);
	const perkId = mp.getIdFromDesc(perk.desc);
...
export const removePerk = (mp: Mp, self: PapyrusObject, args: PapyrusValue[]): void => {
	const perk = getObject(args, 0);
	const selfId = mp.getIdFromDesc(self.desc);
	const perkId = mp.getIdFromDesc(perk.desc);
