...
//   mp.get(0, 'onlinePlayers').forEach((formId) => {
//     if (formId == getNumber(args, 0)) return true;
//   });
//   return false;
// }
...
export const register = (mp: Mp, localization: Localization = localizationDefault): void => {
  for (const className of ['Multiplayer', 'M']) {
    ...
    // mp.registerPapyrusFunction('global', className, 'AsActorEx', (self, args) => asConvert(mp, self, args));
    // mp.registerPapyrusFunction('global', className, 'AsObjectReferenceEx', (self, args) => asConvert(mp, self, args));
