...
import { float32, int32, uint16, uint32, uint8 } from '../../utils/helper';
import { hasKeywordEx } from '../form/keywords';
import { conditionFunctions } from './functionList';
import { ConditionResult, ConditionFlag, ConditionOperator, ConditionRunOn } from './type';

export const conditionAllResult = (mp: Mp, conditionFields: EspmField[], subjectId: number) => {
  ...
  } else if (condResults[0].flag === ConditionFlag.OR) {
...
export const conditionResult = (mp: Mp, cond: ArrayBufferLike, subjectId: number): ConditionResult | undefined => {
  ...
  const param2 = int32(cond, 16);
  const runOn: ConditionRunOn = uint32(cond, 20);
  const ref = uint32(cond, 24);
  // console.log(
