...
const getLookAt = (m: Movement): Actor | null => {
	if (!m.lookAt) return null;
	try {
		return Game.findClosestActor(m.lookAt[0], m.lookAt[1], m.lookAt[2], 128);
	} catch (e) {
		return null;
	}
};
...
export const applyMovement = (refr: ObjectReference, m: Movement): void => {
	...
	const lookAt: Actor | null = getLookAt(m);

	if (lookAt) ac.setLookAt(lookAt, false);
