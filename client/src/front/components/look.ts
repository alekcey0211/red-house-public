...
const applyLookCommon = (look: Look, npc: ActorBase): void => {
	...
	const headparts = look.headpartIds.map((id) => HeadPart.from(Game.getFormEx(id))).filter((headpart) => !!headpart);
...
export const applyLookToPlayer = (look: Look): void => {
	applyLookCommon(look, ActorBase.from((Game.getPlayer() as Actor).getBaseObject()) as ActorBase);
