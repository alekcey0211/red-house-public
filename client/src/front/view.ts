...
import { applyAnimation, setDefaultAnimsDisabled } from './components/animation';
...
export class FormView implements View<FormModel> {
	...
	update(model: FormModel): void {
		...
		if (refId) {
			...
			const base = getFormEx(+(model.baseId as number)) || getFormEx(this.getLookBasedBase());
			if (!base) return;

			let refr = ObjectReference.from(Game.getFormEx(this.refrId));
			const respawnRequired =
				!refr || !refr.getBaseObject() || (refr.getBaseObject() as Form).getFormID() !== base.getFormID();

			if (respawnRequired) {
				this.destroy();
				const player = Game.getPlayer() as Actor;
				refr = player.placeAtMe(base, 1, true, true) as ObjectReference;
				this.state = {};
				...
				new SpawnProcess(
					...
					model.movement ? model.movement.pos : [player.getPositionX(), player.getPositionY(), player.getPositionZ()],
					refr.getFormID(),
					() => {
						this.ready = true;
...
export class WorldView implements View<WorldModel> {
	...
	update(model: WorldModel): void {
		...
		const pcWorldOrCell = player.getWorldSpace() || player.getParentCell();
